...
const pagamentoConfig = {
  ...
  pendente: { label: 'Pendente', bg: 'bg-yellow-50 text-yellow-600' },
  convenio: { label: 'Convênio', bg: 'bg-blue-50 text-blue-600' },
};

const emptyForm = {
  pacienteId: '', profissionalId: '', data: '', horario: '',
  valor: '', observacoes: '', formaPagamento: '', statusPagamento: 'pendente'
};
...
export default function AgendaPage() {
  ...
  const salvarEdicao = async () => {
    setLoading(true);
    try {
      await api.put(`/api/consultas/${editando}`, {
        ...editForm,
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Agenda do Consultório</h1>
          <p className="text-gray-500 text-sm mt-1">Gerencie todas as consultas agendadas</p>
        </div>
        <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={form.pacienteId} onChange={e => setForm(p => ({ ...p, pacienteId: e.target.value }))} required
              ...
              <div>
                ...
                <select value={form.profissionalId} onChange={e => setForm(p => ({ ...p, profissionalId: e.target.value }))} required
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
              ...
              <div>
                ...
                <select value={form.horario} onChange={e => setForm(p => ({ ...p, horario: e.target.value }))} required
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
              ...
              <div>
                ...
                <select value={form.formaPagamento} onChange={e => setForm(p => ({
                  ...p,
                  formaPagamento: e.target.value,
                  statusPagamento: e.target.value === 'convenio' ? 'convenio' : p.statusPagamento,
                }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                  <option value="">Selecione</option>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={editForm.statusPagamento} onChange={e => setEditForm(p => ({ ...p, statusPagamento: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                    <option value="pendente">Pendente</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Forma de Pagamento</label>
                  <select value={editForm.formaPagamento} onChange={e => setEditForm(p => ({
                    ...p,
                    formaPagamento: e.target.value,
                    statusPagamento: e.target.value === 'convenio' ? 'convenio' : p.statusPagamento,
                  }))}
      ...
      <div className="space-y-2">
        {loadingConsultas && <p className="text-gray-400 text-sm text-center py-8">Carregando...</p>}
          ...
          return (
            <div key={c.id} className={`bg-white rounded-xl border ${rowBorder} px-5 py-4 flex items-center gap-4`}>
              ...
              <div className="w-28">
                ...
                <p className="text-sm text-gray-700">{new Date(c.data).toLocaleDateString('pt-BR')}</p>
              </div>
              <div className="w-16">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button title="Editar" onClick={() => abrirEdicao(c)}
                ...
                {!['cancelada', 'realizada'].includes(c.status) && (
                  <>
                    {c.status !== 'confirmada' && (
                      <button title="Confirmar" onClick={() => atualizarStatus(c.id, 'confirmada')}
